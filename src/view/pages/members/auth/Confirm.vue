<template>
  <b-card class="login-body mb-5">
    ...
    <b-card class="mb-5" v-if="!showWarning && !showForm">
      <h3 class="text-center mt-4 mb-4">Ditt konto har blivit verifierat.</h3>
      <div class="text-center">Tack för att du har bekräftat ditt konto.</div>
      <hr>
      <router-link to="/ml-login" tag="a" class="line-link blue-link d-flex flex-row-reverse">login</router-link>
    </b-card>
...
export default {
  ...
  watch: {
    showWarning: function (newValue, oldValue) {
      console.log(this.showWarning, "ShowWarning");
    }
  },
  mounted() {
  ...
  methods: {
    async getClientByToken(code) {
      const res = await axios.get(`/client/resettoken?token=${code}`);
      if (res.status === 204 || res.status.status === 200) {
    ...
    async onSubmit() {
      ...
      if (res.status === 204) {
        ...
        // this.showWarning = true;
        // "Tack för att du har bekräftat ditt konto"
        this.showForm = false;
      } else {
        // false
        this.$func.showTextMessage('Unauthorized', 'Please check your credentials', 'info');
